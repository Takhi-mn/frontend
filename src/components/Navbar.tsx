...
const Navbar = async ({ donate }: Props) => {
  ...
  return (
    <nav className="sm:px-16 px-6 w-full flex items-center py-5 ">
      ...
      items-center max-w-7xl mx-auto bg-zinc-900 px-7 py-2 rounded-xl"
      >
        <Link
          href="/"
        ...
        <ul className="list-none hidden sm:flex flex-row gap-10">
          {menuData.map((nav) => (
            <li
              key={nav.titleEn}
              className={` hover:text-zinc-300 text-white text-[18px] cursor-pointer`}
            >
        ...
        <div className="flex items-center gap-3">
          <Button className="bg-[#FB760D] hover:bg-orange-400 font-normal">
            {donate}
          </Button>
